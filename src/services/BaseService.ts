...
export abstract class BaseService<T> {
     ...
     * @param options
     */
    async update(conditions: any, doc: any, options?: any): Promise<T[]> {
        // conditions.isDeleted = {$ne: 1};
    ...
    async save(doc: T): Promise<T> {
        // return await new this.repository(doc).save()
    ...
    async findOneAndUpdate(doc: T): Promise<T> {
        ...
        return await this.repository.findOneAndUpdate(conditions, doc, { new: true, upsert: true, setDefaultsOnInsert: true}, (err) => {
            console.log(err)
        }).lean()  as T;
    }
     ...
     * @param doc
     */
    async deleteOne(conditions: any): Promise<Object> {
     ...
     * @param doc
     */
    async romove(conditions: any): Promise<Object> {
